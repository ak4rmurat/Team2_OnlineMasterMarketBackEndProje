...
public class Manage {
  ...
  private final String US06_bankAccountVeriGirisi     = "INSERT INTO u201212290_onlinemasterqa.bank_account (id, user_id, user_name, acc_no, bank_name, bank_addr, ifsc_code, pancard_no, paypal_account, paypal_email_id, status) VALUES (?,?,?,?,?,?,?,?,?,?,?);";
  private final String US06_bankAccountStatusActive   = "SELECT * FROM bank_account WHERE status = 'active';";
  private final String US08_inactiveKullaniciSayisi   = "SELECT COUNT(*) AS inactive_count FROM bank_account WHERE status = 'inactive';";
  private final String US08_inactiveKullaniciSil      = "DELETE FROM bank_account WHERE status = 'inactive';";
  private final String US17_enCokMesajGonderenKisi    = "WITH MessageCounts AS (SELECT name, COUNT(*) AS message_count FROM contact_form_details GROUP BY name);";
  private final String US17_enCokAtilanMesaj          = "SELECT name FROM contact_form_details GROUP BY name ORDER BY COUNT(*) DESC LIMIT 1;";
